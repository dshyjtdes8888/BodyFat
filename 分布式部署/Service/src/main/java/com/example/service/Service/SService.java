...
public class SService {
    ...
    public List<BodyFat> findAll() {
        String responseJson = (String) rabbitTemplate.convertSendAndReceive("topic_exchange","ly.uu","findAll");
       // System.out.println("Received JSON response: " + responseJson);

        if(Objects.equals(responseJson, "")){
            return null;
        }
        // 将 JSON 转换为 List<BodyFat>
        List<BodyFat> bodyFatList = deserializeJson(responseJson);
        System.out.println("Deserialized BodyFat list: " + bodyFatList.size());
        return bodyFatList;
    }

    //JSON反序列化
    private List<BodyFat> deserializeJson(String json) {
        ObjectMapper objectMapper = new ObjectMapper();
        try {
            return objectMapper.readValue(json, new TypeReference<List<BodyFat>>() {});
        } catch (IOException e) {
            e.printStackTrace();
            // 处理异常
